...
from classroom.forms import UserForm,TeacherProfileForm,StudentProfileForm,MarksForm,MessageForm,NoticeForm,AssignmentForm,SubmitForm,TeacherProfileUpdateForm,StudentProfileUpdateForm,NotesForm
from django.urls import reverse
from django.contrib.auth.decorators import login_required
from django.contrib.auth import authenticate, login, logout,update_session_auth_hash
from django.http import HttpResponseRedirect,HttpResponse
from classroom import models
from classroom.models import StudentsInClass,StudentMarks,ClassAssignment,SubmitAssignment,Student,Teacher,ClassNotes
...
def upload_notes(request):
    ...
    if request.method == 'POST':
        ...
        form = NotesForm()
    return render(request,'classroom/upload_notes.html',{'form':form,'note_uploaded':note_uploaded})
...
## List of all the notes uploaded by the teacher himself.
@login_required
def note_list(request):
    teacher = request.user.Teacher
    return render(request,'classroom/uploaded_notes.html',{'teacher':teacher})

@login_required
def update_note(request,id=None):
    obj = get_object_or_404(ClassNotes, id=id)
    ...
    if form.is_valid():
        ...
        if 'note' in request.FILES:
            obj.note = request.FILES['note']
        obj.save()
        messages.success(request, "Updated Note".format(obj.assignment_name))
        return redirect('classroom:note_list')
    template = "classroom/update_note.html"
    return render(request, template, context)
...
def note_delete(request, id=None):
    ...
    template = "classroom/note_delete.html"
    return render(request, template, context)

#student view of notes
@login_required
def class_note(request):
    student = request.user.Student
    return render(request,'classroom/class_notes.html',{'student':student,'note_list':note_list})
